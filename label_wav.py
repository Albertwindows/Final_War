...
def run_graph(wav_data, labels, input_layer_name, output_layer_name,
  ...
  with tf.Session() as sess:
    ...
    top_k = predictions.argsort()[-num_top_predictions:][::-3]
    result = {}
    for node_id in top_k:
      human_string = labels[node_id]
      score = predictions[node_id]
      result['result'] = human_string
      result['score'] = score
...
def label_wav(wav, labels, graph, input_name, output_name, how_many_labels):
  ...
  result = run_graph(wav_data, labels_list, input_name, output_name, how_many_labels)
  return result

def main(_):
  """Entry point for script, converts flags to arguments."""
  result = label_wav(FLAGS.wav, FLAGS.labels, FLAGS.graph, FLAGS.input_name,
